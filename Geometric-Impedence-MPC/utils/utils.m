...
classdef utils
    methods(Static)
        %% Plotting %%
        function plot_link( ...
            link_base_spatial_frame_SE3, ...
            link_tail_spatial_frame_SE3, ...
            screw_motion_SE3, ...
            screw_motion_SE3_tail, ...
            link_relative_displacement_R3, ...
            link_base_frame_name, ...
            link_tail_frame_name, ...
            link_color, axis_ref)
            %{
            ...
            before_rigid_body_motion = link_base_spatial_frame_SE3*points_on_link;
            % Now we transform the body from its initial position to its final position
            after_rigid_body_motion = screw_motion_SE3*before_rigid_body_motion;
            ...
            utils.plot_axis(screw_motion_SE3,link_base_spatial_frame_SE3,0.1,'-.',link_base_frame_name)
            
            % Plot the coordinate frame of the tail:
            if size(link_tail_spatial_frame_SE3) == [4,4]
                utils.plot_axis(screw_motion_SE3_tail,link_tail_spatial_frame_SE3,0.1,'-',link_tail_frame_name)
            end
        end
        function plot_axis( ...
            screw_motion_SE3, ...
        ...
        function transformed_origin = plot_Summit(screw_rotation,loc_of_link_frame,link_name,axis_ref)
            ...
            % end
            % Plot the co-ordinate frame of the link at the base:
            utils.plot_axis(screw_rotation,loc_of_link_frame,0.1,'-',link_name)
        end
    end
end