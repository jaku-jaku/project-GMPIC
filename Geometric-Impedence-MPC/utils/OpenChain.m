...
%                 exp_xi_theta_in_SE3_1_{i} = RBT.screw_SE3_from_axis_point_theta(w_R3_0_{i}, q_R3_0_{i}, theta_(i));
        ...
        function g_s_st_SE3_ = compute_Spatial_FK_for_all_joints(exp_xi_theta_in_SE3_1_, g0_st_SE3_)
            ...
            for i=1:N_jnts
                ...
                g_s_st_SE3_{i} = g_st_SE3 * g0_st_SE3_{i};
            end
        end
        function J_s_st = compute_Spatial_Jacobian(xi_R6_1_, g_s_st_SE3_)
             N_jnts = length(xi_R6_1_);
             validate.if_dimension("g_s_st_SE3_", g_s_st_SE3_, [1 N_jnts]);
             J_s_st_ = cell(1,N_jnts);
             % J = [xi_1, xi_2', ... , xi_N' ]; 
             J_s_st_{1} = xi_R6_1_{1};
             for i=2:N_jnts
                % -> compute current jacobian:
                Ad_0_i = Lie.Ad_SE3_from_SE3(g_s_st_SE3_{i-1}); % frame i-1:0
                J_s_st_{i} =  Ad_0_i * xi_R6_1_{i};          % joint   i:1
             end
            J_s_st = cat(2,J_s_st_{:}); % form a 6x6 matrix
        end
        %% Shortcuts:
